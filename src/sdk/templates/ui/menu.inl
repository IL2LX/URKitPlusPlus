...
inline void render_menu() {
    ...
    ImGui::BeginChild("##content", ImVec2(win_size.x - sidebar_w, win_size.y - header_h), ImGuiChildFlags_AlwaysUseWindowPadding);
    ImGui::PushFont(Theme::heading_font());
    ImGui::PushStyleColor(ImGuiCol_Text, p.text_primary);
    ...
    for (int index = 0; index < total_modules; ++index) {
        ...
        if (ModUI::Widgets::toggle(mod->name(), &enabled))
